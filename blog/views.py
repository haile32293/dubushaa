...
class HomeView(View):
    def get(self,request,*args,**kwargs):
        ...
        featured_obj = Blog.objects.all().filter(status='active', visible=True, featured=True).order_by('catagories','-created_at')[:1]
        post_obj = Blog.objects.all().filter(status='active', visible=True)
...
class SingleBlogView(View):
    def get(self,request,id,*args,**kwargs):
        ...
        post_obj.save()
        releted_post = Blog.objects.filter(author=post_obj.author).exclude(id=id).order_by('-id')
        # As per templates views 
        first_post = releted_post.first()
        last_post  = releted_post[:]
